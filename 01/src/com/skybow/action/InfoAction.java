...
import com.skybow.actionSuper.InfoSuperAction;
...
public class InfoAction extends InfoSuperAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5363394168047875981L;
	...
	public String ListShow() {
		...
		CreatePage createPage = myOp.OpCreatePage(sqlFreeAll, params, perR,
		...
		if (currentP == 1) { // 设置显示第1页信息的SQL语句
			sqlFreeSub = "SELECT  * FROM tb_info WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC limit "
					+ top1 + "";
		} else { // 设置显示除第1页外，其他指定页码信息的SQl语句
			int top2 = (currentP - 1) * top1;
			sqlFreeSub = "SELECT  * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date < (SELECT MIN(info_date) FROM (SELECT TOP "
					+ top2
					+ " (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC) AS mindate)) ORDER BY info_date DESC limit "
					+ top1 + "";
			// sqlFreeSub="SELECT TOP "+top1+" * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date NOT IN (SELECT TOP "+top2+" info_date FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC)) ORDER BY info_date DESC";
			// //另一种实现分页查询的SQL语句
		}

		List onefreelist = myOp.OpListShow(sqlFreeSub, params); // 获取免费信息，信息数量由createPage.setPerR(int
																// PerR)方法指定
	...
	public String SingleShow() {
		...
		if (infoSingle == null) {
			...
			addFieldError("SingleShowNoExist",
	...
	public String Add() {
		...
		if (addType.equals("add")) {
			...
			System.out.println("---->" + type + "," + title + "," + content
					+ "," + linkman + "," + phone + "," + email + "," + date
					+ "," + state + "," + payfor);
			String sql = "insert into tb_info(info_type,info_title,info_content,info_linkman,info_phone,info_email,info_date,info_state,info_payfor)values(?,?,?,?,?,?,?,?,?)";
	...
	public String SearchShow() throws UnsupportedEncodingException {
		...
		String sqlSearchAll = "SELECT * FROM tb_info WHERE (" + subsql + opname
		...
		int perR = 8;
		String strCurrentP = request.getParameter("showpage");
		String gowhich = "info_SearchShow.action?searchInfo.subsql=" + subsql
				+ "&searchInfo.sqlvalue=" + sqlvalue + "&searchInfo.type="
				+ type + "&showType=link";

		OpDB myOp = new OpDB();
		CreatePage createPage = myOp.OpCreatePage(sqlSearchAll, params, perR,
				strCurrentP, gowhich); // 调用OpDB类中的OpCreatePage()方法计算出总记录数、总页数，并且设置当前页码，这些信息都封装到了createPage对象中
		...
		if (currentP == 1) { // 显示第1页信息的SQL语句
			sqlSearchSub = "SELECT  * FROM tb_info WHERE (" + subsql + opname
					+ "?) ORDER BY info_date DESC limit " + top1 + "";
		} else { // 显示除第1页外，其他指定页码信息的SQl语句
			int top2 = (currentP - 1) * top1;
			sqlSearchSub = "SELECT TOP "
					+ top1
					+ " * FROM tb_info WHERE ("
					+ subsql
					+ opname
					+ "?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT TOP "
					+ top2
					+ " info_date FROM tb_info WHERE "
					+ subsql
					+ opname
					+ "'"
					+ param
					+ "' ORDER BY info_date DESC) AS mindate)) ORDER BY info_date DESC";
			// sqlSearchSub="SELECT TOP "+top1+" * FROM tb_info WHERE ("+subsql+opname+"?) AND (info_date NOT IN (SELECT TOP "+top2+" info_date FROM tb_info WHERE "+subsql+opname+"'"+param+"' ORDER BY info_date DESC)) ORDER BY info_date DESC";
			// //另一种实现分页查询的SQL语句
		}
	...
	public void validateListShow() {
		...
		if (infoType == null && infoType.equals("")) {
			...
			addFieldError("ListShowNoType",
					getText("city.info.listshow.no.infoType"));
		} else {
			try {
				Integer.parseInt(infoType);
			} catch (NumberFormatException e) {
				System.out.println("信息类别不是数字格式！");
				addFieldError("ListShowWrongType",
						getText("city.info.listshow.format.infoType"));
				e.printStackTrace();
			}
		}
	}
	...
	public void validateSingleShow() {
		...
		if (id == null && id.equals("")) {
			...
			addFieldError("SingleShowNoID",
					getText("city.info.singleshow.no.id"));
		} else {
			try {
				Integer.parseInt(id);
			} catch (NumberFormatException e) {
				System.out.println("信息ID值不是数字格式！");
				addFieldError("ListShowWrongID",
						getText("city.info.listshow.format.infoID"));
				e.printStackTrace();
			}
		}
	}
	...
	public void validateAdd() {
		...
		if (addType.equals("add")) {
			...
			if (content == null || content.equals("")) {
				mark = false;
				addFieldError("contentError",
						getText("city.info.no.infoContent"));
			}
			...
			if (linkman == null || linkman.equals("")) {
				mark = false;
				addFieldError("linkmanError",
						getText("city.info.no.infoLinkman"));
			}
			...
			if (mark) {
				...
				if (phone.indexOf(",") < 0) {
					if (!infoSingle.getInfoPhone().matches(phoneRegex)) {
						addFieldError("phoneError",
								getText("city.info.format.infoPhone"));
					}
				} else {
				...
				if (!infoSingle.getInfoEmail().matches(emailRegex)) {
					addFieldError("emailError",
							getText("city.info.format.infoEmail"));
				}
			}
		}
	}
